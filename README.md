...
## 概要
React + TypeScript で実装した、YouTube 動画のサムネイル一覧からクリックでモーダル再生できるシンプルなデモです。
...
今回のモーダルは「閉じるボタン」と「YouTube プレイヤー」を主なフォーカス対象とした、シンプルな 「動画ビューア」 タイプのため、
...
背景への `inert` 付与 / Escape キー / オーバーレイクリックによる
